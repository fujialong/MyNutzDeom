! function(t, e) {
	...
	var o, n, a, r, i, s, l, u, d, h, 	c, g, m, f = document.getElementsByTagName("body")[0],
		p = document.getElementsByClassName("username")[0],
		v = document.getElementsByClassName("account")[0],
		w = document.getElementsByClassName("app_img")[0],
		y = document.getElementById("app_upload"),
		_ = document.getElementById("upload_bg"),
		C = document.getElementsByClassName("gray_layer")[0],
		E = document.getElementsByClassName("crop_window")[0],
		I = document.getElementsByClassName("crop_main")[0],
		b = document.getElementById("crop_workplace"),
		B = (document.getElementsByClassName("crop_border")[0], document.getElementById("crop_cancel")),
		k = document.getElementById("crop_confirm"),
		T = (document.getElementById("crop_reload"), document.getElementById("hidden_workplace")),
		L = document.getElementById("gray_workplace"),
		A = document.getElementsByClassName("dec_text"),
		D = document.getElementById("type_a"),
		R = document.getElementById("type_b"),
		M = document.getElementsByClassName("edit_area")[0],
		N = document.getElementsByClassName("first_btn")[0],
		P = document.getElementById("boy"),
		x = document.getElementById("girl"),
		U = document.getElementById("obj"),
		O = document.getElementById("image_field"),
		S = (document.getElementById("main_form"), document.getElementById("upload_form")),
		H = document.getElementById("post_img"),
		$ = document.getElementsByClassName("post_img_outer")[0],
		X = document.getElementsByClassName("preview_area_w")[0],
		Q = document.getElementsByClassName("update_btn")[0],
		G = document.getElementsByClassName("close")[0],
		F = document.getElementsByClassName("loading-bar")[0],
		K = document.getElementsByClassName("download")[0],
		z = document.getElementsByClassName("weibo")[0],
		Y = document.getElementsByClassName("weixin_share")[0],
		J = document.getElementsByClassName("share")[0],
		W = document.getElementById("qrcode"),
		j = document.getElementsByClassName("color"),
		Z = document.getElementsByClassName("gender")[0],
		q = document.getElementsByClassName("gender_list")[0],
		V = document.getElementById("callback_frame"),
		tt = document.getElementsByClassName("app_dec")[0],
		et = document.getElementById("zoom"),
		ot = document.getElementsByClassName("zoom")[0],
		nt = document.getElementsByClassName("zoom_bar_blue")[0],
		at = document.getElementsByClassName("zoom_bar")[0],
		rt = document.getElementById("zoom_handle"),
		it = document.getElementById("zoom_out"),
		st = document.getElementById("zoom_in"),
		lt = document.getElementsByClassName("edit")[0],
		ut = document.getElementById("loading_icon"),
		dt = document.getElementsByClassName("img_bg")[0],
		ht = 0,
		ct = 0,
		gt = 1,
		mt = 1,
		...
		Ot = function() {
			...
			var h, m, f, p, v, w = Et.getImageData(0, 0, 2 * o, 2 * n),
				y = w.data.length;
		...
		Ht = function() {
			var t = St();
			if(0 === t.code) c = new Image, c.src = t.data.image, c.onload = Ot;
			else {
				if(1001 === t.errInfo.code) return void(location.href = "www.baidu.com");
		...
		$t = function() {
			if(y.value)
				if(Nt(), this.files) {
					var e = this.files[0];
					if(t.URL && t.URL.createObjectURL) c = new Image, c.src = t.URL.createObjectURL(e), c.onload = Ot;
					else {
		...
		Ft = function() {
			...
			if(Mt(E, "loaded")) {
				...
				var p, v, I, b, B = Et.getImageData(0, 0, 108, 177),
					k = B.data.length;
		...
		zt = function() {
			if(pt) {
				...
				for(var t = 0; 8 > t; t++)
					if(!(ft.dec[t] === A[t].value || (/^爱\s*$/.test(ft.dec[t]) || /^会\s*$/.test(ft.dec[t]) || /^\s*$/.test(ft.dec[t])) && (/^爱\s*$/.test(A[t].value) || /^会\s*$/.test(A[t].value) || /^\s*$/.test(A[t].value)))) return !0;
				return(ft.dec[8] === A[8].value || (/^还爱\s*$/.test(ft.dec[8]) || /^还会\s*$/.test(ft.dec[8]) || /^\s*$/.test(ft.dec[8])) && (/^还爱\s*$/.test(A[8].value) || /^还会\s*$/.test(A[8].value) || /^\s*$/.test(A[8].value))) && (ft.dec[9] === A[9].value || /^\s*$/.test(ft.dec[9]) && /^\s*$/.test(A[9].value)) ? !1 : !0
			}
			return !1
		},
		Yt = function() {
			for(var t = 0; 8 > t; t++) /^爱\s*$/.test(A[t].value) || /^会\s*$/.test(A[t].value) ? Dt(A[t], "default") : Rt(A[t], "default");
			/^还爱\s*$/.test(A[8].value) || /^还会\s*$/.test(A[8].value) ? Dt(A[8], "default") : Rt(A[8], "default");
			for(var t = 0; 9 > t; t++)
				if(A[t].value.length > 18) return Dt(tt, "error"), Rt(M, "update"), Dt(N, "disabled"), Rt(X, "gogogo"), !1;
		...
		Wt = function(t) {
			...
			}, Rt(J, "disabled"), Jt(t.data.image);
			else {
				if(1001 === t.errInfo.code) return void(location.href = "http://localhost:8080/mynutzdeom/index");
		...
		jt = function() {
			...
			if(e) {
				...
				for(var o = 0; 8 > o; o++) t[o] = /^爱\s*$/.test(A[o].value) || /^会\s*$/.test(A[o].value) ? "" : A[o].value;
				t[8] = /^还爱\s*$/.test(A[8].value) || /^还会\s*$/.test(A[8].value) ? "" : A[8].value, t[9] = A[9].value;
				...
				n = "style=" + ft.style + "&word[]=" + encodeURIComponent(ft.dec[0]) + "&word[]=" + encodeURIComponent(ft.dec[1]) + "&word[]=" + encodeURIComponent(ft.dec[2]) + "&word[]=" + encodeURIComponent(ft.dec[3]) + "&word[]=" + encodeURIComponent(ft.dec[4]) + "&word[]=" + encodeURIComponent(ft.dec[5]) + "&word[]=" + encodeURIComponent(ft.dec[6]) + "&word[]=" + encodeURIComponent(ft.dec[7]) + "&word[]=" + encodeURIComponent(ft.dec[8]) + "&identity=" + encodeURIComponent(ft.dec[9]) + "&gender=" + ft.gender + "&color=" + ft.color + "&image=" + encodeURIComponent(ft.img), a.onload = function() {
					...
					Wt(t)
				}, a.open("post", "mynutzdeom/home/getInfo", !0), a.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), a.send(n)
			}
		},
		Zt = function() {
			var t = "",
			...
			e.onload = function(t) {
				...
				if(0 === o.code) p.innerHTML = o.data.name + p.innerHTML, Rt(p, "hidden");
				else {
					if(1001 === o.errInfo.code) return void(location.href = "");
					alert(o.errInfo.msg)
				}
			}, e.open("get", t, !0), e.send()
		},
		qt = function(t) {
			for(var e, o, n = [], e = 0; 7 > e; e++)
				if(j[e].checked) {
					o = e + 1;
					break
				}
			for(e = 0; 8 > e; e++) n[e] = /^爱\s*$/.test(A[e].value) || /^会\s*$/.test(A[e].value) || /^\s*$/.test(A[e].value) ? "" : A[e].value;
			n[8] = /^还爱\s*$/.test(A[8].value) || /^还会\s*$/.test(A[8].value) || /^\s*$/.test(A[8].value) ? "" : A[8].value, n[9] = /^\s*$/.test(A[9].value) ? "" : A[9].value, m = new Image, m.src = "www.baidu.comproud/getMask.php?color=" + o + "&word[]=" + encodeURIComponent(n[0]) + "&word[]=" + encodeURIComponent(n[1]) + "&word[]=" + encodeURIComponent(n[2]) + "&word[]=" + encodeURIComponent(n[3]) + "&word[]=" + encodeURIComponent(n[4]) + "&word[]=" + encodeURIComponent(n[5]) + "&word[]=" + encodeURIComponent(n[6]) + "&word[]=" + encodeURIComponent(n[7]) + "&word[]=" + encodeURIComponent(n[8]) + "&gender=" + gt + "&identity=" + encodeURIComponent(n[9]), m.onload = t
		},
		Vt = function() {
			if(d !== h) {
				var t = ht / 236 * (h - d) + d,
		...
		te = function() {
			f.onclick = function() {
				...
				Z.innerHTML = "体育达人", gt = 1, Yt(), setTimeout(function() {
					Rt(q, "on")
				}, 200), t.stopPropagation()
			}, x.onclick = function(t) {
				Z.innerHTML = "文化达人", gt = 2, Yt(), setTimeout(function() {
					Rt(q, "on")
				}, 200), t.stopPropagation()
			}/*, U.onclick = function(t) {
				Z.innerHTML = "其他达人", gt = 3, Yt(), setTimeout(function() {
					Rt(q, "on")
				}, 200), t.stopPropagation()
			...
			for(var t = 0; 10 > t; t++) ! function(t) {
				A[t].onkeydown = function(e) {
					...
					8 > t && /^\s*$/.test(this.value) ? (this.value = D.checked ? "爱" : "会", vt[0][t] = "爱", vt[1][t] = "会") : 8 === t && /^\s*$/.test(this.value) && (this.value = D.checked ? "还爱" : "还会", vt[0][t] = "还爱", vt[1][t] = "还会"), Yt()
				}, A[t].onkeyup = function(e) {
					return 229 === yt[t] && 32 !== e.keyCode ? void clearTimeout(_t[t]) : (D.checked ? (8 > t ? "爱" !== wt[t] && "" !== wt[t] && "" === this.value && (this.value = "爱") : 8 === t && "还爱" !== wt[t] && "还" !== wt[t] && "" !== wt[t] && "" === this.value && (this.value = "还爱"), vt[0][t] = this.value, wt[t] = this.value, "爱" === this.value.substr(0, 1) && 8 > t ? vt[1][t] = "会" + this.value.substr(1) : 8 > t ? vt[1][t] = this.value : "还爱" === this.value.substr(0, 2) && 8 === t ? vt[1][t] = "还会" + this.value.substr(2) : 8 === t && (vt[1][t] = this.value)) : (8 > t ? "会" !== wt[t] && "" !== wt[t] && "" === this.value && (this.value = "会") : 8 === t && "还会" !== wt[t] && "还" !== wt[t] && "" !== wt[t] && "" === this.value && (this.value = "还会"), vt[1][t] = this.value, wt[t] = this.value, "会" === this.value.substr(0, 1) && 8 > t ? vt[0][t] = "爱" + this.value.substr(1) : 8 > t ? vt[0][t] = this.value : "还会" === this.value.substr(0, 2) && 8 === t ? vt[0][t] = "还爱" + this.value.substr(2) : 8 === t && (vt[0][t] = this.value)), void Yt())
				}, A[t].onpaste = function() {
					D.checked ? (vt[0][t] = this.value, "爱" === this.value.substr(0, 1) && 8 > t ? vt[1][t] = "会" + this.value.substr(1) : 8 > t ? vt[1][t] = "会" + this.value : "还爱" === this.value.substr(0, 2) && 8 === t ? vt[1][t] = "还会" + this.value.substr(2) : 8 === t && (vt[1][t] = "还会" + this.value)) : (vt[1][t] = this.value, "会" === this.value.substr(0, 1) && 8 > t ? vt[0][t] = "爱" + this.value.substr(1) : 8 > t ? vt[0][t] = "爱" + this.value : "还会" === this.value.substr(0, 2) && 8 === t ? vt[0][t] = "还爱" + this.value.substr(2) : 8 === t && (vt[0][t] = "还爱" + this.value)), Yt()
				}
			}(t);
	...
	t.prototype = {
		...
		setupPositionProbePattern: function(t, e) {
			for(var o = -1; 7 >= o; o++)
				if(!(-1 >= t + o || this.moduleCount <= t + o))
					for(var n = -1; 7 >= n; n++) - 1 >= e + n || this.moduleCount <= e + n || (this.modules[t + o][e + n] = o >= 0 && 6 >= o && (0 == n || 6 == n) || n >= 0 && 6 >= n && (0 == o || 6 == o) || o >= 2 && 4 >= o && n >= 2 && 4 >= n ? !0 : !1)
		},
		...
		createMovieClip: function(t, e, o) {
			...
			for(var r = 0; r < this.modules.length; r++)
				for(var i = r * a, s = 0; s < this.modules[r].length; s++) {
					var l = s * a,
						u = this.modules[r][s];
			...
			for(var t = c.getPatternPosition(this.typeNumber), e = 0; e < t.length; e++)
				for(var o = 0; o < t.length; o++) {
					...
					if(null == this.modules[n][a])
						for(var r = -2; 2 >= r; r++)
							for(var i = -2; 2 >= i; i++) this.modules[n + r][a + i] = -2 == r || 2 == r || -2 == i || 2 == i || 0 == r && 0 == i ? !0 : !1
				}
		},
		...
		mapData: function(t, e) {
			for(var o = -1, n = this.moduleCount - 1, a = 7, r = 0, i = this.moduleCount - 1; i > 0; i -= 2)
				for(6 == i && i--;;) {
					for(var s = 0; 2 > s; s++)
		...
		for(var w = new Array(v), y = 0, h = 0; a > h; h++)
			for(var l = 0; l < e.length; l++) h < i[l].length && (w[y++] = i[l][h]);
		for(var h = 0; r > h; h++)
			for(var l = 0; l < e.length; l++) h < s[l].length && (w[y++] = s[l][h]);
		return w
	};
	for(var u = {
			MODE_NUMBER: 1,
			...
			getLostPoint: function(t) {
				for(var e = t.getModuleCount(), o = 0, n = 0; e > n; n++)
					for(var a = 0; e > a; a++) {
						for(var r = 0, i = t.isDark(n, a), s = -1; 1 >= s; s++)
							if(!(0 > n + s || n + s >= e))
								for(var l = -1; 1 >= l; l++) 0 > a + l || a + l >= e || (0 != s || 0 != l) && i == t.isDark(n + s, a + l) && r++;
						r > 5 && (o += 3 + r - 5)
					}
				for(var n = 0; e - 1 > n; n++)
					for(var a = 0; e - 1 > a; a++) {
				...
				for(var n = 0; e > n; n++)
					for(var a = 0; e - 6 > a; a++) t.isDark(n, a) && !t.isDark(n, a + 1) && t.isDark(n, a + 2) && t.isDark(n, a + 3) && t.isDark(n, a + 4) && !t.isDark(n, a + 5) && t.isDark(n, a + 6) && (o += 40);
				for(var a = 0; e > a; a++)
					for(var n = 0; e - 6 > n; n++) t.isDark(n, a) && !t.isDark(n + 1, a) && t.isDark(n + 2, a) && t.isDark(n + 3, a) && t.isDark(n + 4, a) && !t.isDark(n + 5, a) && t.isDark(n + 6, a) && (o += 40);
				for(var d = 0, a = 0; e > a; a++)
					for(var n = 0; e > n; n++) t.isDark(n, a) && d++;
	...
	o.prototype = {
		...
		multiply: function(t) {
			for(var e = new Array(this.getLength() + t.getLength() - 1), n = 0; n < this.getLength(); n++)
				for(var a = 0; a < t.getLength(); a++) e[n + a] ^= g.gexp(g.glog(this.get(n)) + g.glog(t.get(a)));
			return new o(e, 0)
		},
		mod: function(t) {
		...
		for(var a = o.length / 3, r = [], i = 0; a > i; i++)
			for(var s = o[3 * i + 0], l = o[3 * i + 1], u = o[3 * i + 2], d = 0; s > d; d++) r.push(new n(l, u));
		return r
			...
			if(this._android && this._android <= 2.1) {
				...
				CanvasRenderingContext2D.prototype.drawImage = function(t, e, a, r, i, s, l, u, d) {
					if("nodeName" in t && /img/i.test(t.nodeName))
						for(var h = arguments.length - 1; h >= 1; h--) arguments[h] = arguments[h] * o;
